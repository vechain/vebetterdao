...
resource "aws_lambda_function" "relayer_claim_reward" {
  ...
  logging_config {
    log_format = "Text"
    log_group  = "/aws/lambda/relayer-claim-reward"
  }

  environment {
    variables = {
      LAMBDA_ENV = local.network == "mainnet" ? "mainnet" : "testnet-staging"
    }
  }
