resource "aws_iam_role" "scheduler_lambda_execution_role" {
  name = "b3tr-distribute-weekly-app-rewards-${terraform.workspace}-lambda"
  path = "/"
  ...
  tags = {
    ...
    ManagedBy   = "terraform"
    Project     = "distribute-weekly-app-rewards"
  }
}
...
resource "aws_iam_role" "scheduler_eventbridge_execution_role" {
  name = "b3tr-distribute-weekly-app-rewards-${terraform.workspace}-eventbridge-scheduler"
  path = "/"
