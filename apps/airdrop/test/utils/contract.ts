...
export const getOrDeployB3tr = async (
  nodeUrl: string,
  minter: string,
  ...
  try {
    // Deploy the contract
    logger.info("Deploying B3tr contract for testing")
    ...
    logger.info(`B3tr contract deployed at ${address}`)
...
export const getBalance = async (nodeUrl: string, contractAddress: string, address: string): Promise<string> => {
  // Create a thor client
  const thorClient = new ThorClient(new HttpClient(nodeUrl))
