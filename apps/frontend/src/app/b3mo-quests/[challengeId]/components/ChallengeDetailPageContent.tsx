...
export const ChallengeDetailPageContent = ({ challengeId }: { challengeId: string }) => {
  ...
  useEffect(() => {
    if (isFresh && challenge) {
      router.replace(`/b3mo-quests/${challengeId}`)
    }
  ...
  if (!challenge) {
    return (
      <Card.Root variant="primary" p={{ base: "6", md: "8" }} w="full" borderRadius="3xl" boxShadow="sm">
        <VStack gap="3" py="8">
          <Heading size="md">{t("B3MO Quest not found")}</Heading>
          <Button asChild variant="secondary" size="sm">
            <NextLink href="/b3mo-quests">{t("Back to B3MO quests")}</NextLink>
          </Button>
        </VStack>
      </Card.Root>
    )
  ...
  const breadcrumbItems = [
    { label: t("B3MO Quests"), href: "/b3mo-quests" },
    { label: t("#{{id}}", { id: challenge.challengeId }), href: `/b3mo-quests/${challengeId}` },
  ]
