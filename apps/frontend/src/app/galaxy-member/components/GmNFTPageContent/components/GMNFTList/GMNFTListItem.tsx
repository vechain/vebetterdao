...
export const GMNFTListItem: React.FC<GMNFTListItemProps> = ({ token }) => {
  ...
  const isXNodeAttachedAndTransferred = useMemo(() => {
    ...
    return false
  }, [nodeIdAttachedToToken, userXNodeId, isXNodeDelegated, xNodeHasGMAttached])
  ...
  return (
    <>
      ...
      {isXNodeAttachedAndTransferred && (
        // show the detach gm button because the xnode have been transferred to another wallet
        <FeatureFlagWrapper feature={FeatureFlag.GALAXY_MEMBER_UPGRADES} fallback={<></>}>
          <Button
