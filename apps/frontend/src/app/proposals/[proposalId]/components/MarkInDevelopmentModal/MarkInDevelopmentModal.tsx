...
import {
  MarkInDevelopmentPayee,
  useMarkProposalInDevelopmentWithPayees,
} from "@/hooks/useMarkProposalInDevelopmentWithPayees"
...
export const MarkInDevelopmentModal = ({ proposalId, maxBudget, isOpen, onClose }: Props) => {
  ...
  const mutation = useMarkProposalInDevelopmentWithPayees({
    proposalId,
  ...
  const overBudget = sumWei > maxBudget
  const canSubmit =
    !submitting &&
    devNickname.trim().length > 0 &&
    discussionLink.trim().length > 0 &&
    parsedPayees.length > 0 &&
    parsedPayees.length === rows.length &&
    !overBudget
  ...
  }, [canSubmit, mutation, parsedPayees, devNickname, discussionLink])
