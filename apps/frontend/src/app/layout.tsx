// Force per-request rendering so metadata (basePath) and window.__ENV__ resolve
// against runtime env vars, not whatever was set at `next build` time.
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  // `</script>` and break out of the inline script tag below.
  const runtimeEnvJson = JSON.stringify(
    Object.fromEntries(PUBLIC_ENV_KEYS.map(k => [k, process.env[k] ?? ""])),
  ).replace(/</g, "\\u003c")
  return (
    <html
