...
export const EmptyState = React.forwardRef<HTMLDivElement, EmptyStateProps>(function EmptyState(props, ref) {
  ...
  return (
    <ChakraEmptyState.Root ref={ref} rounded="md" bg="bg.tertiary" {...rest}>
      <ChakraEmptyState.Content>
        {icon && <ChakraEmptyState.Indicator>{icon}</ChakraEmptyState.Indicator>}
        {description ? (
          <VStack textAlign="center" color="text.subtle">
            <ChakraEmptyState.Title color="text.subtle">{title}</ChakraEmptyState.Title>
            <ChakraEmptyState.Description>{description}</ChakraEmptyState.Description>
          </VStack>
        ) : (
          <ChakraEmptyState.Title textAlign="center" color="text.subtle">
            {title}
