...
import { Button, Card, Field, HStack, Icon, Link, NumberInput, Text, VStack } from "@chakra-ui/react"
...
export const PowerUpContent = ({ onClose }: Props) => {
    ...
    <VStack gap={5} w="full">
      <VStack gap={2} justify={"center"} w="full">
        ...
        <Text textStyle="md" color="text.subtle" textAlign="center">
          {t("for the next round using B3TR.")}
        </Text>
      ...
      {Number(availableBalance) < 2 && (
        <Card.Root w="full" mt={2} p={3} bg="card.default" border="1px solid" borderColor="border.secondary" rounded="xl">
          <HStack gap={3}>
