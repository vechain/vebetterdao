 ...
 * via the new {markAsInDevelopmentWithPayees} entrypoint. Callable by the proposer or
 * any wallet holding PROPOSAL_STATE_MANAGER_ROLE.
 */
export const useMarkProposalInDevelopmentWithPayees = ({ proposalId, onSuccess, transactionModalCustomUI }: Props) => {
    ...
    ({ payees, devNickname, discussionLink }: SendArgs) => {
      ...
      return [
        buildClause({
          ...
          method: "markAsInDevelopmentWithPayees",
