...
import { getAppEndorsementScoreQueryKey, getEndorsersQueryKey } from "@/api"
...
export const useEndorseApp = ({ appId, nodeId, onSuccess }: Props) => {
  ...
  const refetchQueryKeys = useMemo(() => [getEndorsersQueryKey(appId), getAppEndorsementScoreQueryKey(appId)], [appId])
