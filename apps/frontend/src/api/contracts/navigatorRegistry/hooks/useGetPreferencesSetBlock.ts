...
const method = "getPreferencesSetBlock" as const

export const getGetPreferencesSetBlockQueryKey = (navigator: string, roundId: string) =>
  getCallClauseQueryKeyWithArgs({ abi, address, method, args: [navigator as `0x${string}`, BigInt(roundId)] })

export const useGetPreferencesSetBlock = (roundId?: string) => {
