...
const method = "exitAnnouncedRound" as const

export const getExitAnnouncedRoundQueryKey = (account: string) =>
  getCallClauseQueryKeyWithArgs({ abi, address, method, args: [account as `0x${string}`] })

export const useExitAnnouncedRound = (account?: string) => {
  ...
  return useCallClause({
    ...
    queryOptions: {
      ...
      select: data => (data?.[0] as bigint) ?? 0n,
    },
  })
}
