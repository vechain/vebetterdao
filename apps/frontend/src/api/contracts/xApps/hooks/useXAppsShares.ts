...
export const useXAppsShares = (apps: string[], roundId?: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const contract = thor.contracts.load(getConfig().xAllocationPoolContractAddress, XAllocationPool__factory.abi)
      const clauses = apps.map(app => contract.clause.getAppShares(roundId, app))
      const res = await thor.transactions.executeMultipleClausesCall(clauses)
      if (!res.every(r => r.success)) throw new Error(`Failed to fetch xApps shares for ${apps} in round ${roundId}`)
