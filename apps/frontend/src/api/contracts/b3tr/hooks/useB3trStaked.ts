...
/**
 * Fetches the staked balance of a B3TR token for a given address.
 *
 * @param {Connex.Thor} thor - The Connex instance.
 * @param {string} [address] - The address to fetch the staked balance for.
 * @param {number} [scaleDecimals=18] - The number of decimals to scale the balance by.
 *
 * @returns {Promise<TokenBalance>} The staked balance of the B3TR token.
 *
 * @throws {Error} If the address is not provided or if there is a VM error.
 */
export const getB3trStaked = async (
  thor: Connex.Thor,
  address?: string,
  ...
  const functionFragment = VOT3__factory.createInterface().getFunction("stakedBalanceOf").format("json")
...
/**
 * Returns a unique query key for the staked balance of a B3TR token.
 *
 * @param {string} [address] - The address to get the query key for.
 *
 * @returns {string[]} The query key.
 */
export const getStakedB3TRQueryKey = (address?: string) => ["staked", "b3tr", address]

/**
 * Returns the staked balance of a B3TR token for a given address.
 *
 * @param {string} [address] - The address to get the balance for.
 *
 * @returns {UseQueryResult<TokenBalance, unknown>} The staked balance of the B3TR token.
 */
export const useB3trStaked = (address?: string) => {
  const { thor } = useConnex()
  const { data: tokenDetails } = useB3trTokenDetails()

  return useQuery({
    queryKey: getStakedB3TRQueryKey(address),
    queryFn: () => getB3trStaked(thor, address, tokenDetails?.decimals),
