...
import { getRoundXAppsQueryKey } from "@vechain/vechain-kit"
import { formatEther } from "viem"
import { ThorClient } from "@vechain/sdk-network"
import { getRoundXApps } from "../../xApps"
...
export const getXAppRoundEarnings = async (
  ...
  const res = await thor.contracts
    .load(getConfig().xAllocationPoolContractAddress, XAllocationPool__factory.abi)
    .read.roundEarnings(roundId, xAppId)

  if (!res) throw new Error(`Failed to fetch xApp round earnings for ${xAppId} in round ${roundId}`)
...
export const useXAppRoundEarnings = (roundId: string, xAppId: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const data = await getRoundXApps(thor, roundId)
...
export const useMultipleXAppRoundEarnings = (roundId: string, xAppIds: string[]) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const contract = thor.contracts.load(xAllocationPoolContract, XAllocationPool__factory.abi)
      const clauses = xAppsInRound.map(app => contract.clause.roundEarnings(roundId, app.id))

      const res = await thor.transactions.executeMultipleClausesCall(clauses)

      if (!res.every(r => r.success)) throw new Error("Failed to fetch xApp round earnings")

      const decoded = res.map((earnings, index) => {
        const parsedAmount = formatEther(earnings.result.plain as bigint)
