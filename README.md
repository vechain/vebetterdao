...
It is possible to simulate x-app voting rounds. Simply set the `RUN_SIMULATION` flag in `packages/config/contracts/envs.local.ts` to `true`. Then run a clean version of the app using `make solo-down && make solo-up && yarn start`. The simulation will only work against thor solo in your local environment.
