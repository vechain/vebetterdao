...
/**
 * Generates a mock e2e config file if it does not exist yet.
 * Required so that hardhat (and any other module that does `getConfig()` for env=e2e)
 * can resolve the import even before the real deploy runs.
 *
 * The real `e2e.ts` is overwritten by `overrideLocalConfigWithNewContracts` after a
 * fresh deploy against the e2e Thor solo (port 8670).
 */
export const generateMockE2EConfig = () => {
  console.log("Checking if @repo/config/e2e.ts exists...")
  const e2eConfigPath = path.resolve("./e2e.ts")
  if (fs.existsSync(e2eConfigPath)) {
    console.log(`${e2eConfigPath} exists, skipping...`)
    return
  }

  console.log(`${e2eConfigPath} does not exist, generating mock...`)
  const toWrite = `import { AppConfig } from "." \n const config: AppConfig = {
    ...
    navigatorRegistryContractAddress: "0x45d5CA3f295ad8BCa291cC4ecd33382DE40E4FAc",
  ...
  console.log(`Writing mock e2e config file to ${e2eConfigPath}`)
  fs.writeFileSync(e2eConfigPath, toWrite)
  console.log("Done!")
}

generateMockE2EConfig()
