...
library GovernorVotesLogicV5 {
  ...
  function _countVote(
    ...
    GovernorTypesV5.ProposalVote storage proposalVote = self.proposalVotes[proposalId];
    ...
    if (support == uint8(GovernorTypesV5.VoteType.Against)) {
      proposalVote.againstVotes += vote;
    } else if (support == uint8(GovernorTypesV5.VoteType.For)) {
      proposalVote.forVotes += vote;
    } else if (support == uint8(GovernorTypesV5.VoteType.Abstain)) {
  ...
  function castVote(
    ...
    GovernorStateLogicV5.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV5.encodeStateBitmap(GovernorTypesV5.ProposalState.Active)
    );
   ...
   * @param weight - The weight of the vote.
   */
  function _checkVotingThreshold(GovernorStorageTypesV5.GovernorStorage storage self, uint256 weight) private view {
   ...
   */
  function toggleQuadraticVoting(GovernorStorageTypesV5.GovernorStorage storage self) external {
