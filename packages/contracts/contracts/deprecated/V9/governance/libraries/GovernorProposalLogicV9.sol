...
import { GovernorStateLogicV9 } from "./GovernorStateLogicV9.sol";
import { GovernorClockLogicV9 } from "./GovernorClockLogicV9.sol";
import { GovernorDepositLogicV9 } from "./GovernorDepositLogicV9.sol";
import { GovernorGovernanceLogicV9 } from "./GovernorGovernanceLogicV9.sol";
import { GovernorFunctionRestrictionsLogicV9 } from "./GovernorFunctionRestrictionsLogicV9.sol";
...
library GovernorProposalLogicV9 {
   ...
   */
  event ProposalCreatedWithType(uint256 indexed proposalId, GovernorTypesV9.ProposalType proposalType);
   ...
   */
  error GovernorInvalidProposalType(GovernorTypesV9.ProposalType proposalType);
   ...
   */
  error GovernorRestrictedProposal(uint256 proposalId, GovernorTypesV9.ProposalType proposalType);
   ...
   */
  function canProposalStartInNextRound(GovernorStorageTypesV9.GovernorStorage storage self) external view returns (bool) {
  ...
  function proposalType(
    GovernorStorageTypesV9.GovernorStorage storage self,
    uint256 proposalId
  ) external view returns (GovernorTypesV9.ProposalType) {
  ...
  function propose(
    ...
    validateProposeParams(
      ...
      calldatas,
      proposalId,
      GovernorTypesV9.ProposalType.Standard
    );
  ...
  function proposeGrant(
    ...
    validateProposeParams(
      ...
      calldatas,
      proposalId,
      GovernorTypesV9.ProposalType.Grant
    );
  ...
  function queue(
    ...
    GovernorStateLogicV9.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Succeeded)
    );
  ...
  function execute(
    ...
    GovernorStateLogicV9.validateStateBitmap(
      ...
      proposalId,
      GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Succeeded) |
        GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Queued)
    );
    ...
    if (GovernorGovernanceLogicV9.executor(self) != contractAddress) {
    ...
    if (GovernorGovernanceLogicV9.executor(self) != contractAddress && !self.governanceCall.empty()) {
  ...
  function cancel(
    ...
    GovernorStateLogicV9.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV9.ALL_PROPOSAL_STATES_BITMAP ^
        GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Canceled) ^
        GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Executed) ^
        GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.InDevelopment) ^
        GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Completed) ^
        GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.DepositNotMet) ^
        GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Defeated)
    );

    if (account == proposalProposer(self, proposalId)) {
      require(
        GovernorStateLogicV9._state(self, proposalId) == GovernorTypesV9.ProposalState.Pending,
   ...
   */
  function markAsInDevelopment(GovernorStorageTypesV9.GovernorStorage storage self, uint256 proposalId) external {
    GovernorTypesV9.ProposalType proposalType = self.proposalType[proposalId];
    GovernorTypesV9.ProposalCore storage proposal = self.proposals[proposalId];

    // Only Standard proposals are allowed here.
    // Proposals created before v7 (when proposalType mapping was introduced) will default to Standard.
    if (proposalType != GovernorTypesV9.ProposalType.Standard) {
    ...
    if (proposal.isExecutable && GovernorStateLogicV9._state(self, proposalId) == GovernorTypesV9.ProposalState.Succeeded) {
      revert GovernorRestrictedProposal(proposalId, proposalType);
    }

    self.proposalDevelopmentState[proposalId] = GovernorTypesV9.ProposalDevelopmentState.InDevelopment;
    //Emit event
   ...
   */
  function markAsCompleted(GovernorStorageTypesV9.GovernorStorage storage self, uint256 proposalId) external {
    GovernorTypesV9.ProposalType proposalType = self.proposalType[proposalId];
    ...
    self.proposalDevelopmentState[proposalId] = GovernorTypesV9.ProposalDevelopmentState.Completed;
    //Emit event
   ...
   */
  function resetDevelopmentState(GovernorStorageTypesV9.GovernorStorage storage self, uint256 proposalId) external {
    GovernorStateLogicV9.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.InDevelopment) |
    ...
    self.proposalDevelopmentState[proposalId] = GovernorTypesV9.ProposalDevelopmentState.PendingDevelopment;
    //Emit event
  ...
  function _propose(
    ...
    uint256 depositThresholdAmount = GovernorDepositLogicV9._depositThresholdByProposalType(self, proposalTypeValue);
    ...
    if (depositAmount > 0) {
      GovernorDepositLogicV9.depositFunds(self, depositAmount, proposer, proposalId);
    }
  ...
  function validateProposeParams(
    ...
    if (proposalTypeValue == GovernorTypesV9.ProposalType.Standard) {
    ...
    if (self.proposals[proposalId].roundIdVoteStart != 0) {
      ...
      revert GovernorUnexpectedProposalState(proposalId, GovernorStateLogicV9._state(self, proposalId), bytes32(0));
    }

    GovernorFunctionRestrictionsLogicV9.checkFunctionsRestriction(self, targets, calldatas);
  }
  ...
  function _executeOperations(
    ...
    self.timelock.executeBatch{ value: msg.value }(
      ...
      calldatas,
      0,
      GovernorGovernanceLogicV9.timelockSalt(descriptionHash, contractAddress)
    );
  ...
  function _queueOperations(
    ...
    bytes32 salt = GovernorGovernanceLogicV9.timelockSalt(descriptionHash, contractAddress);
  ...
  function _canProposalStartInNextRound(
    ...
    uint48 currentBlock = GovernorClockLogicV9.clock(self);
  ...
  function _proposalSnapshot(
    ...
    if (currentRoundDeadline <= GovernorClockLogicV9.clock(self)) {
      currentRoundDeadline = GovernorClockLogicV9.clock(self);
    }
   ...
   */
  function isValidProposalType(GovernorTypesV9.ProposalType proposalTypeValue) internal pure returns (bool) {
    return
      proposalTypeValue == GovernorTypesV9.ProposalType.Standard || proposalTypeValue == GovernorTypesV9.ProposalType.Grant;
  }
