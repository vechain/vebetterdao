...
import { GovernorConfiguratorV9 } from "./GovernorConfiguratorV9.sol";
...
library GovernorDepositLogicV9 {
   ...
   */
  function deposit(GovernorStorageTypesV9.GovernorStorage storage self, uint256 amount, uint256 proposalId) external {
    if (amount == 0) {
    ...
    if (proposal.proposer == msg.sender && self.proposalType[proposalId] == GovernorTypesV9.ProposalType.Grant) {
    ...
    GovernorStateLogicV9.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Pending)
    );
   ...
   */
  function withdraw(GovernorStorageTypesV9.GovernorStorage storage self, uint256 proposalId, address depositer) external {
    ...
    uint208 currentVotes = self.depositsVotingPower[depositer].upperLookupRecent(GovernorClockLogicV9.clock(self));
    uint208 newVotes = SafeCast.toUint208(currentVotes - amount);
    self.depositsVotingPower[depositer].push(GovernorClockLogicV9.clock(self), newVotes);
  ...
  function depositFunds(
    ...
    uint208 currentVotes = self.depositsVotingPower[depositor].upperLookupRecent(GovernorClockLogicV9.clock(self));
    uint208 newVotes = currentVotes + SafeCast.toUint208(amount);
    self.depositsVotingPower[depositor].push(GovernorClockLogicV9.clock(self), newVotes);
  ...
  function _depositThresholdByProposalType(
    GovernorStorageTypesV9.GovernorStorage storage self,
    GovernorTypesV9.ProposalType proposalType
  ) internal view returns (uint256) {
    uint256 percentageBasedThreshold = (GovernorConfiguratorV9.getDepositThresholdPercentage(self, proposalType) *
      self.b3tr.totalSupply()) / 100;
    uint256 maxThreshold = GovernorConfiguratorV9.getDepositThresholdCap(self, proposalType);
