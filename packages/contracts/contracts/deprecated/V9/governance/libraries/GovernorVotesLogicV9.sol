...
library GovernorVotesLogicV9 {
  ...
  function _countVote(
    ...
    GovernorTypesV9.ProposalVote storage proposalVote = self.proposalVotes[proposalId];
    ...
    if (support == uint8(GovernorTypesV9.VoteType.Against)) {
      proposalVote.againstVotes += vote;
    } else if (support == uint8(GovernorTypesV9.VoteType.For)) {
      proposalVote.forVotes += vote;
    } else if (support == uint8(GovernorTypesV9.VoteType.Abstain)) {
   ...
   */
  function userVotedOnce(GovernorStorageTypesV9.GovernorStorage storage self, address user) internal view returns (bool) {
  ...
  function castVote(
    ...
    GovernorStateLogicV9.validateStateBitmap(
      self,
      proposalId,
      GovernorStateLogicV9.encodeStateBitmap(GovernorTypesV9.ProposalState.Active)
    );

    uint256 proposalSnapshot = GovernorProposalLogicV9._proposalSnapshot(self, proposalId);
    ...
    GovernorTypesV9.ProposalType proposalType = GovernorProposalLogicV9.proposalType(self, proposalId);
  ...
  function _checkVotingThreshold(
    ...
    uint256 threshold = GovernorConfiguratorV9.getVotingThreshold(self, proposalType);
    if (weight < threshold) {
   ...
   */
  function toggleQuadraticVoting(GovernorStorageTypesV9.GovernorStorage storage self) external {
    bool isQuadraticDisabled = self.quadraticVotingDisabled.upperLookupRecent(GovernorClockLogicV9.clock(self)) == 1; // 0: enabled, 1: disabled
    ...
    self.quadraticVotingDisabled.push(GovernorClockLogicV9.clock(self), newStatus);
