...
import { IXAllocationPoolV2 } from "./interfaces/IXAllocationPoolV2.sol";
...
import { ITreasuryV2 } from "./interfaces/ITreasuryV2.sol";
import { IEmissionsV2 } from "./interfaces/IEmissionsV2.sol";
 ...
 */
contract XAllocationPoolV2 is IXAllocationPoolV2, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
  ...
  struct XAllocationPoolStorage {
    IXAllocationVotingGovernorV2 _xAllocationVoting;
    IEmissionsV2 _emissions;
    IB3TRV2 b3tr;
    ITreasuryV2 treasury;
    IX2EarnAppsV2 x2EarnApps;
    IX2EarnRewardsPoolV2 x2EarnRewardsPool;
  ...
  function initialize(
    ...
    require(_b3trAddress != address(0), "XAllocationPoolV2: new b3tr is the zero address");
    require(_treasury != address(0), "XAllocationPoolV2: new treasury is the zero address");
    require(_x2EarnApps != address(0), "XAllocationPoolV2: new x2EarnApps is the zero address");
    require(_x2EarnRewardsPool != address(0), "XAllocationPoolV2: new x2EarnRewardsPool is the zero address");
    ...
    $.b3tr = IB3TRV2(_b3trAddress);
    $.treasury = ITreasuryV2(_treasury);
    $.x2EarnApps = IX2EarnAppsV2(_x2EarnApps);
    $.x2EarnRewardsPool = IX2EarnRewardsPoolV2(_x2EarnRewardsPool);

    require(_admin != address(0), "XAllocationPoolV2: new admin is the zero address");
  ...
  function setXAllocationVotingAddress(address xAllocationVoting_) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(xAllocationVoting_ != address(0), "XAllocationPoolV2: new xAllocationVoting is the zero address");

    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    $._xAllocationVoting = IXAllocationVotingGovernorV2(xAllocationVoting_);
  ...
  function setEmissionsAddress(address emissions_) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(emissions_ != address(0), "XAllocationPoolV2: new emissions is the zero address");
  ...
  function setTreasuryAddress(address treasury_) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(treasury_ != address(0), "XAllocationPoolV2: new treasury is the zero address");

    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    $.treasury = ITreasuryV2(treasury_);
  ...
  function setX2EarnAppsAddress(address x2EarnApps_) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(x2EarnApps_ != address(0), "XAllocationPoolV2: new x2EarnApps is the zero address");
  ...
  function claim(uint256 roundId, bytes32 appId) external nonReentrant {
    ...
    require(!$.claimedRewards[appId][roundId], "XAllocationPoolV2: rewards already claimed for this app and round");
    require(!xAllocationVoting().isActive(roundId), "XAllocationPoolV2: round not ended yet");
    require($.x2EarnApps.appExists(appId), "XAllocationPoolV2: app does not exist");
    ...
    require(amountToClaim > 0, "XAllocationPoolV2: no rewards available for this app");
    ...
    require(
      ...
      "XAllocationPoolV2: Insufficient funds on contract"
    );
    ...
    require(
      ...
      "XAllocationPoolV2: transfer to team wallet failed"
    );
    ...
    require(
      ...
      "XAllocationPoolV2: Approval of B3TR token to x2EarnRewardsPool failed"
    );
    require(
      $.x2EarnRewardsPool.deposit(x2EarnRewardsPoolAmount, appId),
      "XAllocationPoolV2: Deposit of rewards allocation to x2EarnRewardsPool failed"
    );
    ...
    if (unallocatedAmount > 0) {
      require(
        ...
        "XAllocationPoolV2: Transfer of unallocated rewards to treasury failed"
      );
    }
  ...
  {
    IXAllocationVotingGovernorV2 _xAllocationVoting = xAllocationVoting();

    require(_xAllocationVoting != IXAllocationVotingGovernorV2(address(0)), "XAllocationVotingGovernor contract not set");
    ...
    IXAllocationVotingGovernorV2.RoundState state = _xAllocationVoting.state(roundId);
    if (
      state == IXAllocationVotingGovernorV2.RoundState.Active || state == IXAllocationVotingGovernorV2.RoundState.Succeeded
    ) {
   ...
   */
  function xAllocationVoting() public view returns (IXAllocationVotingGovernorV2) {
    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
   ...
   * @dev Returns the emissions contract.
   */
  function treasury() external view returns (ITreasuryV2) {
    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    return $.treasury;
  }
   ...
   */
  function b3tr() external view returns (IB3TRV2) {
    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    return $.b3tr;
  }
