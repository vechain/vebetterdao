...
import { X2EarnAppsDataTypesV2 } from "../../libraries/X2EarnAppsDataTypesV2.sol";

/**
 * @title AppsStorageUpgradeableV2
...
abstract contract AppsStorageUpgradeableV2 is Initializable, X2EarnAppsUpgradeableV2 {
  ...
  struct AppsStorageStorage {
    ...
    mapping(bytes32 appId => X2EarnAppsDataTypesV2.App) _apps;
  ...
  function _addApp(
    ...
    // Store the new app
    $._apps[id] = X2EarnAppsDataTypesV2.App(id, appName, block.timestamp);
    $._appIds.push(id);
   ...
   */
  function _getAppStorage(bytes32 appId) internal view returns (X2EarnAppsDataTypesV2.App memory) {
    if (!appExists(appId)) {
  ...
  function app(
    bytes32 appId
  ) public view virtual override returns (X2EarnAppsDataTypesV2.AppWithDetailsReturnType memory) {
    X2EarnAppsDataTypesV2.App memory _app = _getAppStorage(appId);

    return
      X2EarnAppsDataTypesV2.AppWithDetailsReturnType(
        _app.id,
   ...
   */
  function apps() external view returns (X2EarnAppsDataTypesV2.AppWithDetailsReturnType[] memory) {
    AppsStorageStorage storage $ = _getAppsStorageStorage();

    uint256 length = $._appIds.length;
    X2EarnAppsDataTypesV2.AppWithDetailsReturnType[] memory allApps = new X2EarnAppsDataTypesV2.AppWithDetailsReturnType[](
      length
    );

    for (uint i = 0; i < length; i++) {
      X2EarnAppsDataTypesV2.App memory _app = $._apps[$._appIds[i]];
      allApps[i] = X2EarnAppsDataTypesV2.AppWithDetailsReturnType(
        _app.id,
   ...
   */
  function getPaginatedApps(uint startIndex, uint count) external view returns (X2EarnAppsDataTypesV2.App[] memory) {
    AppsStorageStorage storage $ = _getAppsStorageStorage();
    ...
    X2EarnAppsDataTypesV2.App[] memory paginatedApps = new X2EarnAppsDataTypesV2.App[](endIndex - startIndex);
