...
import "./interfaces/IVOT3V2.sol";
import "./interfaces/IERC1155V2.sol";

/// @title TreasuryV2 Contract for VeBetter DAO
  ...
  UUPSUpgradeable
{
  ...
  /// @notice Storage structure for TreasuryV2
  ...
  /// @dev The slot for TreasuryV2 storage in contract storage
  ...
  modifier onlyGovernanceWhenNotPaused() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "TreasuryV2: caller is not a governance actor");
    require(!paused(), "TreasuryV2: contract is paused");
    _;
  }
  ...
  modifier onlyAdminOrGovernance() {
    require(
      ...
      "TreasuryV2: caller is not an admin or governance actor"
    );
    _;
  }
  ...
  function initialize(
    ...
    require(_admin != address(0), "TreasuryV2: admin address cannot be zero");
  ...
  function _validateAddresses(address _b3tr, address _vot3) private {
    require(_b3tr != address(0), "TreasuryV2: B3TR address cannot be zero");
    require(_vot3 != address(0), "TreasuryV2: VOT3 address cannot be zero");
  ...
  /// @notice Pauses the TreasuryV2 contract
  ...
  /// @notice Unpauses the TreasuryV2 contract allowing normal operations
  ...
  function transferVTHO(address _to, uint256 _value) external onlyGovernanceWhenNotPaused {
    ...
    require($.transferLimit[VTHO] >= _value, "TreasuryV2: transfer limit exceeded");

    IERC20V2 vtho = _getERC20Contract(VTHO);

    require(vtho.balanceOf(address(this)) >= _value, "TreasuryV2: insufficient VTHO balance");
    require(vtho.transfer(_to, _value), "TreasuryV2: transfer failed");
  }
  ...
  function transferB3TR(address _to, uint256 _value) external onlyGovernanceWhenNotPaused {
    ...
    require($.transferLimit[$.B3TR] >= _value, "TreasuryV2: transfer limit exceeded");

    IERC20V2 b3tr = _getERC20Contract(b3trAddress());
    require(b3tr.balanceOf(address(this)) >= _value, "TreasuryV2: insufficient B3TR balance");
    require(b3tr.transfer(_to, _value), "TreasuryV2: transfer failed");
  }
  ...
  function transferVOT3(address _to, uint256 _value) external onlyGovernanceWhenNotPaused {
    ...
    require($.transferLimit[$.VOT3] >= _value, "TreasuryV2: transfer limit exceeded");

    IERC20V2 vot3 = _getERC20Contract(vot3Address());
    require(vot3.balanceOf(address(this)) >= _value, "TreasuryV2: insufficient VOT3 balance");
    require(vot3.transfer(_to, _value), "TreasuryV2: transfer failed");
  }
  ...
  function transferVET(address _to, uint256 _value) external onlyGovernanceWhenNotPaused nonReentrant {
    ...
    require($.transferLimitVET >= _value, "TreasuryV2: transfer limit exceeded");

    require(address(this).balance >= _value, "TreasuryV2: insufficient VET balance");
  ...
  function transferTokens(address _token, address _to, uint256 _value) external onlyGovernanceWhenNotPaused {
    ...
    require($.transferLimit[_token] >= _value, "TreasuryV2: transfer limit exceeded");

    IERC20V2 token = _getERC20Contract(_token);
    require(token.balanceOf(address(this)) >= _value, "TreasuryV2: insufficient balance");
    require(token.transfer(_to, _value), "TreasuryV2: transfer failed");
  }
  ...
  function transferNFT(address _nft, address _to, uint256 _tokenId) external onlyGovernanceWhenNotPaused {
    IERC721V2 nft = IERC721V2(_nft);
    require(nft.ownerOf(_tokenId) == address(this), "TreasuryV2: DAO does not own the NFT");
  ...
  function transferERC1155Tokens(
    ...
    IERC1155V2 erc1155 = IERC1155V2(_tokenAddress);
    require(erc1155.balanceOf(address(this), _id) > 0, "TreasuryV2: DAO does not own this ERC1155 token");
  ...
  function convertB3TR(uint256 _b3trAmount) external onlyGovernanceWhenNotPaused {
    IERC20V2 b3tr = _getERC20Contract(b3trAddress());
    IVOT3V2 vot3 = IVOT3V2(vot3Address());
    require(b3tr.balanceOf(address(this)) >= _b3trAmount, "TreasuryV2: insufficient B3TR balance");
    require(b3tr.approve(vot3Address(), _b3trAmount), "TreasuryV2: approval for VOT3 failed");
  ...
  function convertVOT3(uint256 _vot3Amount) external onlyGovernanceWhenNotPaused {
    IVOT3V2 vot3 = IVOT3V2(vot3Address());
    require(vot3.convertedB3trOf(address(this)) >= _vot3Amount, "TreasuryV2: insufficient B3TR converted");
  ...
  function getERC1155TokenBalance(address _token, uint256 _id) external view returns (uint256) {
    IERC1155V2 erc1155 = IERC1155V2(_token);
  ...
  function _getERC20Contract(address token) internal pure returns (IERC20V2) {
    return IERC20V2(token);
  }
