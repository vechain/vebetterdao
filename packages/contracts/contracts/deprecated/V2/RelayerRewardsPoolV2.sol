...
import "./interfaces/IRelayerRewardsPoolV2.sol";
 ...
 */
contract RelayerRewardsPoolV2 is
  AccessControlUpgradeable,
  ReentrancyGuardUpgradeable,
  UUPSUpgradeable,
  IRelayerRewardsPoolV2
{
   ...
   * @param caller The relayer attempting to perform the action
   */
  function _validatePreferredRelayer(address voter, address caller) internal view {
    RelayerRewardsPoolStorage storage $ = _getRelayerRewardsPoolStorage();
    address preferredRelayer = $.preferredRelayer[voter];

    // A stale preference should not block service, so we only enforce it while the relayer stays registered.
    if (preferredRelayer != address(0) && $.registeredRelayers[preferredRelayer] && caller != preferredRelayer) {
      revert NotPreferredRelayer(caller, preferredRelayer);
    }
  }
  ...
  function validateVoteDuringEarlyAccess(uint256 roundId, address voter, address caller) external view {
    ...
    _validatePreferredRelayer(voter, caller);
  }
  ...
  function unregisterRelayer(address relayer) external {
    if (relayer != msg.sender && !hasRole(POOL_ADMIN_ROLE, msg.sender) && !hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
  ...
  /**
   * @notice Set or clear the caller's preferred relayer for early access actions
   * @param relayer The preferred relayer, or zero address to clear it
   */
  function setPreferredRelayer(address relayer) external {
    RelayerRewardsPoolStorage storage $ = _getRelayerRewardsPoolStorage();

    if (relayer != address(0) && !$.registeredRelayers[relayer]) {
      revert RelayerNotRegistered(relayer);
    }

    $.preferredRelayer[msg.sender] = relayer;

    emit PreferredRelayerSet(msg.sender, relayer);
  }
