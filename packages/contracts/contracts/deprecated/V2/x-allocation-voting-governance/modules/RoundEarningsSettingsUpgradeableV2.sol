...
/**
 * @title RoundEarningsSettingsUpgradeableV2
 * @notice Extension of {XAllocationVotingGovernorV2} to handle the settings for the x-allocation earnings calculations:
 ...
 */
abstract contract RoundEarningsSettingsUpgradeableV2 is Initializable, XAllocationVotingGovernorV2 {
  ...
  function _setBaseAllocationPercentage(uint256 baseAllocationPercentage_) internal {
    require(
      ...
      "XAllocationVotingGovernorV2: Base allocation percentage must be less than or equal to 100"
    );
    EarningsSettingsStorage storage $ = _getEarningsSettingsStorage();
  ...
  function _setAppSharesCap(uint256 appSharesCap_) internal {
    require(appSharesCap_ <= 100, "XAllocationVotingGovernorV2: App shares cap must be less than or equal to 100");
    EarningsSettingsStorage storage $ = _getEarningsSettingsStorage();
