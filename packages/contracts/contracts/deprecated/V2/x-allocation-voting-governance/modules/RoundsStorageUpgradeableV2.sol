...
/**
 * @title RoundsStorageUpgradeableV2
 * @dev Extension of {XAllocationVotingGovernorV2} for storing rounds data and managing the rounds lifecycle.
 */
abstract contract RoundsStorageUpgradeableV2 is Initializable, XAllocationVotingGovernorV2 {
  struct RoundCore {
    address proposer;
    uint48 voteStart;
    uint32 voteDuration;
  ...
  function getAppsOfRound(
    uint256 roundId
  ) external view returns (X2EarnAppsDataTypesV2.AppWithDetailsReturnType[] memory) {
    RoundsStorageStorage storage $ = _getRoundsStorageStorage();
