...
import { IXAllocationVotingGovernorV2, IERC6372 } from "../interfaces/IXAllocationVotingGovernorV2.sol";
...
/**
 * @title XAllocationVotingGovernorV2
 ...
 */
abstract contract XAllocationVotingGovernorV2 is
  Initializable,
  ContextUpgradeable,
  ERC165Upgradeable,
  IXAllocationVotingGovernorV2
{
  ...
  function startNewRound() public virtual returns (uint256) {
    ...
    if (currentRound > 0) {
      require(!isActive(currentRound), "XAllocationVotingGovernorV2: there can be only one round per time");
    }
  ...
  function castVote(uint256 roundId, bytes32[] memory appIds, uint256[] memory voteWeights) public virtual {
    ...
    require(appIds.length == voteWeights.length, "XAllocationVotingGovernorV2: apps and weights length mismatch");
    require(appIds.length > 0, "XAllocationVotingGovernorV2: no apps to vote for");
  ...
  function supportsInterface(
    ...
    return interfaceId == type(IXAllocationVotingGovernorV2).interfaceId || super.supportsInterface(interfaceId);
  }
   ...
   */
  function voterRewards() public view virtual returns (IVoterRewardsV2);
}
