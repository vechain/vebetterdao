...
import "./x-allocation-voting-governance/XAllocationVotingGovernorV2.sol";
import "./x-allocation-voting-governance/modules/RoundVotesCountingUpgradeableV2.sol";
import "./x-allocation-voting-governance/modules/VotesUpgradeableV2.sol";
import "./x-allocation-voting-governance/modules/VotesQuorumFractionUpgradeableV2.sol";
import "./x-allocation-voting-governance/modules/VotingSettingsUpgradeableV2.sol";
import "./x-allocation-voting-governance/modules/RoundEarningsSettingsUpgradeableV2.sol";
import "./x-allocation-voting-governance/modules/RoundFinalizationUpgradeableV2.sol";
import "./x-allocation-voting-governance/modules/RoundsStorageUpgradeableV2.sol";
import "./x-allocation-voting-governance/modules/ExternalContractsUpgradeableV2.sol";
import "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

/**
 * @title XAllocationVotingV2
 ...
 */
contract XAllocationVotingV2 is
  XAllocationVotingGovernorV2,
  VotingSettingsUpgradeableV2,
  RoundVotesCountingUpgradeableV2,
  VotesUpgradeableV2,
  VotesQuorumFractionUpgradeableV2,
  RoundEarningsSettingsUpgradeableV2,
  ExternalContractsUpgradeableV2,
  RoundsStorageUpgradeableV2,
  RoundFinalizationUpgradeableV2,
  ...
  UUPSUpgradeable
{
  ...
  function initialize(InitializationData memory data) public initializer {
    require(address(data.vot3Token) != address(0), "XAllocationVotingV2: invalid VOT3 token address");
    require(address(data.voterRewards) != address(0), "XAllocationVotingV2: invalid VoterRewards address");
    require(address(data.emissions) != address(0), "XAllocationVotingV2: invalid Emissions address");
    
    __XAllocationVotingGovernor_init("XAllocationVotingV2");
    ...
    for (uint256 i; i < data.admins.length; i++) {
      require(data.admins[i] != address(0), "XAllocationVotingV2: invalid admin address");
   ...
   */
  function setVoterRewardsAddress(IVoterRewardsV2 newVoterRewards) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
  ...
  function votingPeriod() public view override(XAllocationVotingGovernorV2, VotingSettingsUpgradeableV2) returns (uint256) {
    return super.votingPeriod();
  }

  function quorum(
    uint256 blockNumber
  ) public view override(XAllocationVotingGovernorV2, VotesQuorumFractionUpgradeableV2) returns (uint256) {
    return super.quorum(blockNumber);
  }

  function supportsInterface(
    bytes4 interfaceId
  ) public view override(AccessControlUpgradeable, XAllocationVotingGovernorV2) returns (bool) {
