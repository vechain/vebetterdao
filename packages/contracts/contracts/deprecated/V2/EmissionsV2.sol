...
import "./interfaces/IB3TRV2.sol";
import "./interfaces/IXAllocationVotingGovernorV2.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

/// @title EmissionsV2 Distribution Contract
...
contract EmissionsV2 is AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
  ...
  /// @notice Initialization data for the EmissionsV2 contract
  ...
  function initialize(InitializationData memory data) external initializer {
    // Assertions
    require(data.destinations.length == 4, "EmissionsV2: Invalid destinations input length. Expected 4.");
    require(data.initialXAppAllocation > 0, "EmissionsV2: Initial xApp allocation must be greater than 0");
    require(data.cycleDuration > 0, "EmissionsV2: Cycle duration must be greater than 0");
    require(data.decaySettings.length == 4, "EmissionsV2: Invalid decay settings input length. Expected 4.");
    require(
      data.treasuryPercentage > 0 && data.treasuryPercentage <= 10000,
      "EmissionsV2: Treasury percentage must be between 1 and 10000"
    );
    require(
      data.decaySettings[0] > 0 && data.decaySettings[0] <= 100,
      "EmissionsV2: xAllocations decay must be between 1 and 100"
    );
    require(
      data.decaySettings[1] > 0 && data.decaySettings[1] <= 100,
      "EmissionsV2: vote2Earn decay must be between 1 and 100"
    );
    require(data.decaySettings[2] > 0, "EmissionsV2: xAllocations decay delay must be greater than 0");
    require(data.decaySettings[3] > 0, "EmissionsV2: vote2Earn decay delay must be greater than 0");
    require(
      data.maxVote2EarnDecay > 0 && data.maxVote2EarnDecay <= 100,
      "EmissionsV2: Max vote2Earn decay must be between 0 and 100"
    );

    require(data.destinations[0] != address(0), "EmissionsV2: XAllocations destination cannot be zero address");
    require(data.destinations[1] != address(0), "EmissionsV2: Vote2Earn destination cannot be zero address");
    require(data.destinations[2] != address(0), "EmissionsV2: Treasury destination cannot be zero address");

    require(data.admin != address(0), "EmissionsV2: Admin address cannot be zero address");
  ...
  function bootstrap() external onlyRole(MINTER_ROLE) nonReentrant {
    EmissionsStorage storage $ = _getEmissionsStorage();
    require($.nextCycle == 0, "EmissionsV2: Can only bootstrap emissions when next cycle = 0");
    $.nextCycle++;
  ...
  function start() external onlyRole(MINTER_ROLE) nonReentrant {
    EmissionsStorage storage $ = _getEmissionsStorage();
    require($.b3tr.paused() == false, "EmissionsV2: B3TR token is paused");
    require($.nextCycle == 1, "EmissionsV2: Can only start emissions when next cycle = 1");
  ...
  function distribute() external nonReentrant {
    EmissionsStorage storage $ = _getEmissionsStorage();
    require($.nextCycle > 1, "EmissionsV2: Please start emissions first");
    require(isNextCycleDistributable(), "EmissionsV2: Next cycle not started yet");
    ...
    require(
      ...
      "EmissionsV2: emissions would exceed B3TR supply cap"
    );
  ...
  function _calculateVote2EarnDecayPeriods() internal view returns (uint256) {
    ...
    require($.nextCycle > 0, "EmissionsV2: Invalid cycle number");
    if ($.nextCycle == 1) {
      return 0;
    }
  ...
  function getXAllocationAmount(uint256 cycle) public view returns (uint256) {
    ...
    require(cycle <= $.nextCycle, "EmissionsV2: Cycle not reached yet");
  ...
  function isCycleEnded(uint256 cycle) public view returns (bool) {
    require(cycle <= getCurrentCycle(), "EmissionsV2: Cycle not reached yet");
  ...
  function getCurrentCycle() public view returns (uint256) {
    EmissionsStorage storage $ = _getEmissionsStorage();
    require($.nextCycle > 0, "EmissionsV2: not bootstrapped yet");
    return $.nextCycle - 1;
  }
  ...
  function b3tr() public view returns (IB3TRV2) {
    return _getEmissionsStorage().b3tr;
  }

  /// @notice Returns the XAllocations Governance contract
  function xAllocationsGovernor() public view returns (IXAllocationVotingGovernorV2) {
  ...
  function setXallocationsAddress(address xAllocationAddress) public onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(xAllocationAddress != address(0), "EmissionsV2: xAllocationAddress cannot be the zero address");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setVote2EarnAddress(address vote2EarnAddress) public onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(vote2EarnAddress != address(0), "EmissionsV2: vote2EarnAddress cannot be the zero address");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setTreasuryAddress(address treasuryAddress) public onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(treasuryAddress != address(0), "EmissionsV2: treasuryAddress cannot be the zero address");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setCycleDuration(uint256 _cycleDuration) public onlyRole(DECAY_SETTINGS_MANAGER_ROLE) {
    require(_cycleDuration > 0, "EmissionsV2: Cycle duration must be greater than 0");
    EmissionsStorage storage $ = _getEmissionsStorage();
    require(
      IXAllocationVotingGovernorV2($.xAllocationsGovernor).votingPeriod() < _cycleDuration,
      "EmissionsV2: Voting period must be less than cycle duration"
    );
  ...
  function setXAllocationsDecay(uint256 _decay) public onlyRole(DECAY_SETTINGS_MANAGER_ROLE) {
    require(_decay <= 100, "EmissionsV2: xAllocations decay must be between 0 and 100");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setVote2EarnDecay(uint256 _decay) public onlyRole(DECAY_SETTINGS_MANAGER_ROLE) {
    require(_decay <= 100, "EmissionsV2: vote2Earn decay must be between 0 and 100");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setXAllocationsDecayPeriod(uint256 _period) public onlyRole(DECAY_SETTINGS_MANAGER_ROLE) {
    require(_period > 0, "EmissionsV2: xAllocations decay period must be greater than 0");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setVote2EarnDecayPeriod(uint256 _period) public onlyRole(DECAY_SETTINGS_MANAGER_ROLE) {
    require(_period > 0, "EmissionsV2: vote2Earn decay period must be greater than 0");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setTreasuryPercentage(uint256 _percentage) public onlyRole(DECAY_SETTINGS_MANAGER_ROLE) {
    require(_percentage <= 10000, "EmissionsV2: Treasury percentage must be between 0 and 10000");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setMaxVote2EarnDecay(uint256 _maxVote2EarnDecay) public onlyRole(DECAY_SETTINGS_MANAGER_ROLE) {
    require(_maxVote2EarnDecay <= 100, "EmissionsV2: Max vote2Earn decay must be between 0 and 100");
    EmissionsStorage storage $ = _getEmissionsStorage();
  ...
  function setXAllocationsGovernorAddress(
    ...
    require(_xAllocationsGovernor != address(0), "EmissionsV2: _xAllocationsGovernor cannot be the zero address");
    require(
      IXAllocationVotingGovernorV2(_xAllocationsGovernor).votingPeriod() < cycleDuration(),
      "EmissionsV2: Voting period must be less than cycle duration"
    );
