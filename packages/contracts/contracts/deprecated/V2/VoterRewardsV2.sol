...
import "./interfaces/IGalaxyMemberV2.sol";
import "./interfaces/IB3TRGovernorV2.sol";
import "./interfaces/IXAllocationVotingGovernorV2.sol";
import "./interfaces/IEmissionsV2.sol";
 ...
 * - CONTRACTS_ADDRESS_MANAGER_ROLE: The role that can set the addresses of the contracts used by the VoterRewardsV2 contract.
 */
contract VoterRewardsV2 is AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
  ...
  /// @notice The role that can set the addresses of the contracts used by the VoterRewardsV2 contract.
  ...
  /// @notice Initialize the VoterRewardsV2 contract.
  ...
  function initialize(
    ...
    require(_galaxyMember != address(0), "VoterRewardsV2: _galaxyMember cannot be the zero address");
    require(_emissions != address(0), "VoterRewardsV2: emissions cannot be the zero address");
    require(_b3tr != address(0), "VoterRewardsV2: _b3tr cannot be the zero address");

    require(levels.length > 0, "VoterRewardsV2: levels must have at least one element");
    require(levels.length == multipliers.length, "VoterRewardsV2: levels and multipliers must have the same length");
    ...
    require(admin != address(0), "VoterRewardsV2: admin cannot be the zero address");
    _grantRole(DEFAULT_ADMIN_ROLE, admin);
    _grantRole(UPGRADER_ROLE, upgrader);
    _grantRole(CONTRACTS_ADDRESS_MANAGER_ROLE, contractsAddressManager);
  }

  /// @notice Upgrade the implementation of the VoterRewardsV2 contract.
  ...
  function registerVote(
    ...
    require(proposalStart > 0, "VoterRewardsV2: proposalStart must be greater than 0");
    require(voter != address(0), "VoterRewardsV2: voter cannot be the zero address");
  ...
  function claimReward(uint256 cycle, address voter) external nonReentrant {
    require(cycle > 0, "VoterRewardsV2: cycle must be greater than 0");
    require(voter != address(0), "VoterRewardsV2: voter cannot be the zero address");
    VoterRewardsStorage storage $ = _getVoterRewardsStorage();

    // Check if the cycle has ended before claiming rewards.
    require($.emissions.isCycleEnded(cycle), "VoterRewardsV2: cycle must be ended");

    // Get the reward for the voter in the cycle.
    uint256 reward = getReward(cycle, voter);

    require(reward > 0, "VoterRewardsV2: reward must be greater than 0");
    require($.b3tr.balanceOf(address(this)) >= reward, "VoterRewardsV2: not enough B3TR in the contract to pay reward");

    // Reset the reward-weighted votes for the voter in the cycle.
    $.cycleToVoterToTotal[cycle][voter] = 0;

    // transfer reward to voter
    require($.b3tr.transfer(voter, reward), "VoterRewardsV2: transfer failed");
  ...
  function getReward(uint256 cycle, address voter) public view returns (uint256) {
    ...
    require(emissionsAmount > 0, "VoterRewardsV2: emissionsAmount must be greater than 0");
  ...
  function setLevelToMultiplier(uint256 level, uint256 multiplier) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(level > 0, "VoterRewardsV2: level must be greater than 0");
    require(multiplier > 0, "VoterRewardsV2: multiplier must be greater than 0");
