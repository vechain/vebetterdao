...
library GovernorQuorumLogicV3 {
  ...
  /// @notice Retrieves the latest quorum numerator using the GovernorClockLogicV3 library.
  /// @param self The storage structure containing the quorum numerator history.
  /// @return The latest quorum numerator.
  function quorumNumerator(GovernorStorageTypesV3.GovernorStorage storage self) public view returns (uint256) {
   ...
   */
  function quorum(GovernorStorageTypesV3.GovernorStorage storage self, uint256 timepoint) public view returns (uint256) {
  ...
  function updateQuorumNumerator(
    ...
    self.quorumNumeratorHistory.push(GovernorClockLogicV3.clock(self), SafeCast.toUint208(newQuorumNumerator));
  ...
  function quorumReached(
    ...
    return
      quorum(self, GovernorProposalLogicV3._proposalSnapshot(self, proposalId)) <= self.proposalTotalVotes[proposalId];
  }
}
