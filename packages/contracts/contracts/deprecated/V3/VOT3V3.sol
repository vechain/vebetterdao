...
/// @notice This contract governs the issuance and management of VOT3V3 tokens, which are the tokens used for voting in the VeBetter DAO Ecosystem.
contract VOT3V3 is
  ERC20Upgradeable,
  ...
  UUPSUpgradeable
{
  ...
  /// @notice Storage structure for VOT3V3 contract
  ...
  struct VOT3Storage {
    ...
    mapping(address account => uint256) _convertedB3TR; // Mapping of B3TR tokens converted to VOT3V3 tokens
  }

  /// @dev The slot for VOT3V3 storage in contract storage
  ...
  /// @notice Initializes the VOT3V3 token
  ...
  function initialize(address _admin, address _upgrader, address _pauser, address _b3tr) external initializer {
    __ERC20_init("VOT3V3", "VOT3V3");
    __ERC20Pausable_init();
    __AccessControl_init();
    __ERC20Permit_init("VOT3V3");
    __ERC20Votes_init();
    ...
    require(_admin != address(0), "VOT3V3: Admin address cannot be 0");
    // Grant the contract deployer the default admin role and the UPGRADER_ROLE
    _grantRole(DEFAULT_ADMIN_ROLE, _admin);
    _grantRole(UPGRADER_ROLE, _upgrader);
    _grantRole(PAUSER_ROLE, _pauser);

    require(_b3tr != address(0), "VOT3V3: B3TR address cannot be 0");
    $.b3tr = IERC20(_b3tr);
  }

  /// @notice Pauses the VOT3V3 contract
  ...
  /// @notice Unpauses the VOT3V3 contract
  /// @dev Only callable by the admin role
  ...
  /// @notice Convert B3TR tokens in exchange for VOT3V3 tokens
  /// @dev Converts B3TR tokens and mints VOT3V3 tokens in return
  ...
  /// @notice Convert VOT3V3 previously converted back to B3TR tokens
  /// @dev Burns VOT3V3 tokens and transfers B3TR tokens in return
  /// @param amount Amount of VOT3V3 tokens to convert
  ...
  /// @notice Transfer VOT3V3 tokens to a specific address
  /// @dev Override of the ERC20 transferFrom function
  /// @param to Address to transfer to
  /// @param value Amount of VOT3V3 tokens to transfer
  /// @return True if transfer was successful
  function transfer(address to, uint256 value) public override(ERC20Upgradeable) returns (bool) {
    return super.transfer(to, value);
  }

  /// @notice Approve the transfer of VOT3V3 tokens to a specific address
  /// @dev Override of the ERC20 transferFrom function
  /// @param spender Address to approve
  /// @param value Amount of VOT3V3 tokens to approve
  ...
  /// @notice Transfer VOT3V3 tokens from one address to another
  /// @dev Override of the ERC20 transferFrom function
  ...
  function delegate(address delegatee) public override {
    require(paused() == false, "VOT3V3: contract is paused");
