...
import { IXAllocationVotingGovernorV3, IERC6372 } from "../interfaces/IXAllocationVotingGovernorV3.sol";
...
/**
 * @title XAllocationVotingGovernorV3
  ...
  IXAllocationVotingGovernorV3
{
  ...
  function startNewRound() public virtual returns (uint256) {
    ...
    if (currentRound > 0) {
      require(!isActive(currentRound), "XAllocationVotingGovernorV3: there can be only one round per time");
    }
  ...
  function castVote(uint256 roundId, bytes32[] memory appIds, uint256[] memory voteWeights) public virtual {
    ...
    require(appIds.length == voteWeights.length, "XAllocationVotingGovernorV3: apps and weights length mismatch");
    require(appIds.length > 0, "XAllocationVotingGovernorV3: no apps to vote for");
   ...
   */
  function x2EarnApps() public view virtual returns (IX2EarnAppsV3);

  /**
   * @dev Returns the Emissions contract.
   */
  function emissions() public view virtual returns (IEmissionsV3);

  /**
   * @dev Returns the VoterRewards contract.
   */
  function voterRewards() public view virtual returns (IVoterRewardsV3);
}
