...
/**
 * @title RoundsStorageUpgradeableV3
 * @dev Extension of {XAllocationVotingGovernorV3} for storing rounds data and managing the rounds lifecycle.
 */
abstract contract RoundsStorageUpgradeableV3 is Initializable, XAllocationVotingGovernorV3 {
  struct RoundCore {
    address proposer;
    uint48 voteStart;
    uint32 voteDuration;
  ...
  function getAppsOfRound(
    uint256 roundId
  ) external view returns (X2EarnAppsDataTypesV3.AppWithDetailsReturnType[] memory) {
    RoundsStorageStorage storage $ = _getRoundsStorageStorage();
