  ...
  ReentrancyGuardUpgradeable
{
  ...
  function initialize(
    ...
    require(_admin != address(0), "X2EarnRewardsPoolV3: admin is the zero address");
    require(_contractsManagerAdmin != address(0), "X2EarnRewardsPoolV3: contracts manager admin is the zero address");
    require(_upgrader != address(0), "X2EarnRewardsPoolV3: upgrader is the zero address");
    require(address(_b3tr) != address(0), "X2EarnRewardsPoolV3: b3tr is the zero address");
    require(address(_x2EarnApps) != address(0), "X2EarnRewardsPoolV3: x2EarnApps is the zero address");
  ...
  /**
   * @dev See {IX2EarnRewardsPoolV3-deposit}
   */
  function deposit(uint256 amount, bytes32 appId) external returns (bool) {
    X2EarnRewardsPoolStorage storage $ = _getX2EarnRewardsPoolStorage();

    // check that app exists
    require($.x2EarnApps.appExists(appId), "X2EarnRewardsPoolV3: app does not exist");

    // increase available amount for the app
    $.availableFunds[appId] += amount;

    // transfer tokens to this contract
    require($.b3tr.transferFrom(msg.sender, address(this), amount), "X2EarnRewardsPoolV3: deposit transfer failed");
  ...
  /**
   * @dev See {IX2EarnRewardsPoolV3-withdraw}
   */
  function withdraw(uint256 amount, bytes32 appId, string memory reason) external nonReentrant {
    ...
    require(
      ...
      "X2EarnRewardsPoolV3: not an app admin nor a reward distributor"
    );

    // check if the app has enough available funds to withdraw
    require($.availableFunds[appId] >= amount, "X2EarnRewardsPoolV3: app has insufficient funds");

    // check if the contract has enough funds
    require($.b3tr.balanceOf(address(this)) >= amount, "X2EarnRewardsPoolV3: insufficient funds on contract");
    ...
    $.availableFunds[appId] -= amount;
    require($.b3tr.transfer(teamWalletAddress, amount), "X2EarnRewardsPoolV3: Allocation transfer to app failed");

    emit TeamWithdrawal(amount, appId, teamWalletAddress, msg.sender, reason);
  }

  /**
   * @dev See {IX2EarnRewardsPoolV3-distributeReward}
   */
  function distributeReward(
    ...
    require($.x2EarnApps.isRewardDistributor(appId, msg.sender), "X2EarnRewardsPoolV3: not a reward distributor");
    ...
    $.availableFunds[appId] -= amount;
    require($.b3tr.transfer(receiver, amount), "X2EarnRewardsPoolV3: Allocation transfer to app failed");
   ...
   */
  function setX2EarnApps(IX2EarnAppsV3 _x2EarnApps) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(address(_x2EarnApps) != address(0), "X2EarnRewardsPoolV3: x2EarnApps is the zero address");
  ...
  /**
   * @dev See {IX2EarnRewardsPoolV3-availableFunds}
   */
  ...
  /**
   * @dev See {IX2EarnRewardsPoolV3-version}
   */
  ...
  receive() external payable virtual {
    revert("X2EarnRewardsPoolV3: contract does not accept VET");
  }
  ...
  fallback() external payable {
    revert("X2EarnRewardsPoolV3: contract does not accept calls/data");
  }
  ...
  function onERC721Received(address, address, uint256, bytes memory) public virtual returns (bytes4) {
    revert("X2EarnRewardsPoolV3: contract does not accept ERC721 tokens");
  }
  ...
  function onERC1155Received(address, address, uint256, uint256, bytes memory) public virtual returns (bytes4) {
    revert("X2EarnRewardsPoolV3: contract does not accept ERC1155 tokens");
  }
  ...
  function onERC1155BatchReceived(
    ...
    revert("X2EarnRewardsPoolV3: contract does not accept batch transfers of ERC1155 tokens");
  }
}
