...
import "./x-allocation-voting-governance/modules/RoundVotesCountingUpgradeableV3.sol";
import "./x-allocation-voting-governance/modules/VotesUpgradeableV3.sol";
import "./x-allocation-voting-governance/modules/VotesQuorumFractionUpgradeableV3.sol";
import "./x-allocation-voting-governance/modules/VotingSettingsUpgradeableV3.sol";
import "./x-allocation-voting-governance/modules/RoundEarningsSettingsUpgradeableV3.sol";
import "./x-allocation-voting-governance/modules/RoundFinalizationUpgradeableV3.sol";
import "./x-allocation-voting-governance/modules/RoundsStorageUpgradeableV3.sol";
import "./x-allocation-voting-governance/modules/ExternalContractsUpgradeableV3.sol";
import "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

/**
 * @title XAllocationVotingV3
...
contract XAllocationVotingV3 is
  ...
  VotingSettingsUpgradeableV3,
  RoundVotesCountingUpgradeableV3,
  VotesUpgradeableV3,
  VotesQuorumFractionUpgradeableV3,
  RoundEarningsSettingsUpgradeableV3,
  ExternalContractsUpgradeableV3,
  RoundsStorageUpgradeableV3,
  RoundFinalizationUpgradeableV3,
  ...
  UUPSUpgradeable
{
  ...
  struct InitializationData {
    ...
    IX2EarnAppsV3 x2EarnAppsAddress;
  ...
  function initialize(InitializationData memory data) public initializer {
    require(address(data.vot3Token) != address(0), "XAllocationVotingV3: invalid VOT3 token address");
    require(address(data.voterRewards) != address(0), "XAllocationVotingV3: invalid VoterRewards address");
    require(address(data.emissions) != address(0), "XAllocationVotingV3: invalid Emissions address");
    
    __XAllocationVotingGovernor_init("XAllocationVotingV3");
    ...
    for (uint256 i; i < data.admins.length; i++) {
      require(data.admins[i] != address(0), "XAllocationVotingV3: invalid admin address");
   ...
   */
  function setX2EarnAppsAddress(IX2EarnAppsV3 newX2EarnApps) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
   ...
   */
  function setEmissionsAddress(IEmissionsV3 newEmissions) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
   ...
   */
  function setVoterRewardsAddress(IVoterRewardsV3 newVoterRewards) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
  ...
  function votingPeriod() public view override(XAllocationVotingGovernorV3, VotingSettingsUpgradeableV3) returns (uint256) {
    return super.votingPeriod();
  }

  function quorum(
    uint256 blockNumber
  ) public view override(XAllocationVotingGovernorV3, VotesQuorumFractionUpgradeableV3) returns (uint256) {
