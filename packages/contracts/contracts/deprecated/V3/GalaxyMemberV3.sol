...
/// @title GalaxyMemberV3 NFT Contract
  ...
  UUPSUpgradeable
{
  ...
  /// @notice Storage structure for GalaxyMemberV3
  ...
  struct GalaxyMemberStorage {
    ...
    IB3TRGovernorV3 b3trGovernor; // B3TRGovernor contract
  ...
  /// @notice Initializes a new GalaxyMemberV3 contract
  ...
  function upgrade(uint256 tokenId) external nonReentrant whenNotPaused {
    ...
    require($.b3tr.transferFrom(msg.sender, $.treasury, b3trRequired), "GalaxyMemberV3: Transfer failed");
  ...
  function setB3trGovernorAddress(address _b3trGovernor) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    ...
    $.b3trGovernor = IB3TRGovernorV3(payable(_b3trGovernor));
  }
  ...
  function participatedInGovernance(address user) public view returns (bool) {
    GalaxyMemberStorage storage $ = _getGalaxyMemberStorage();
    require(
      $.xAllocationsGovernor != IXAllocationVotingGovernorV3(address(0)),
      "Galaxy Member: XAllocationVotingGovernor not set"
    );
    require($.b3trGovernor != IB3TRGovernorV3(payable(address(0))), "Galaxy Member: B3TRGovernor not set");
  ...
  function xAllocationsGovernor() external view returns (IXAllocationVotingGovernorV3) {
    GalaxyMemberStorage storage $ = _getGalaxyMemberStorage();
    return $.xAllocationsGovernor;
  }

  /// @notice Gets the b3trGovernor contract address
  function b3trGovernor() external view returns (IB3TRGovernorV3) {
    GalaxyMemberStorage storage $ = _getGalaxyMemberStorage();
    return $.b3trGovernor;
  }
