...
import { X2EarnAppsDataTypesV3 } from "../../libraries/X2EarnAppsDataTypesV3.sol";
...
abstract contract AppsStorageUpgradeableV3 is Initializable, X2EarnAppsUpgradeableV3 {
  ...
  struct AppsStorageStorage {
    ...
    mapping(bytes32 appId => X2EarnAppsDataTypesV3.App) _apps;
  ...
  function _addApp(
    ...
    // Store the new app
    $._apps[id] = X2EarnAppsDataTypesV3.App(id, appName, block.timestamp);
    $._appIds.push(id);
   ...
   */
  function _getAppStorage(bytes32 appId) internal view returns (X2EarnAppsDataTypesV3.App memory) {
    if (!appExists(appId)) {
  ...
  // ---------- Getters ---------- //
  /**
   * @dev See {IX2EarnAppsV3-appExists}.
   */
  ...
  /**
   * @dev See {IX2EarnAppsV3-app}.
   *
   * @param appId the id of the app
   */
  function app(
    bytes32 appId
  ) public view virtual override returns (X2EarnAppsDataTypesV3.AppWithDetailsReturnType memory) {
    X2EarnAppsDataTypesV3.App memory _app = _getAppStorage(appId);

    return
      X2EarnAppsDataTypesV3.AppWithDetailsReturnType(
        _app.id,
  ...
  /**
   * @dev See {IX2EarnAppsV3-apps}.
   *
   * @notice This function could not be efficient with a large number of apps, in that case, use {IX2EarnAppsV3-getPaginatedApps}
   * and then call {IX2EarnAppsV3-app} for each app id
   */
  function apps() external view returns (X2EarnAppsDataTypesV3.AppWithDetailsReturnType[] memory) {
    AppsStorageStorage storage $ = _getAppsStorageStorage();

    uint256 length = $._appIds.length;
    X2EarnAppsDataTypesV3.AppWithDetailsReturnType[] memory allApps = new X2EarnAppsDataTypesV3.AppWithDetailsReturnType[](
      length
    );

    for (uint i = 0; i < length; i++) {
      X2EarnAppsDataTypesV3.App memory _app = $._apps[$._appIds[i]];
      allApps[i] = X2EarnAppsDataTypesV3.AppWithDetailsReturnType(
        _app.id,
  ...
  /**
   * @dev See {IX2EarnAppsV3-getPaginatedApps}.
   */
  function getPaginatedApps(uint startIndex, uint count) external view returns (X2EarnAppsDataTypesV3.App[] memory) {
    AppsStorageStorage storage $ = _getAppsStorageStorage();
    ...
    X2EarnAppsDataTypesV3.App[] memory paginatedApps = new X2EarnAppsDataTypesV3.App[](endIndex - startIndex);
  ...
  /**
   * @dev See {IX2EarnAppsV3-appsCount}.
   */
