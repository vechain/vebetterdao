...
import "./interfaces/IGalaxyMemberV3.sol";
import "./interfaces/IB3TRGovernorV3.sol";
import "./interfaces/IXAllocationVotingGovernorV3.sol";
import "./interfaces/IEmissionsV3.sol";
 ...
 * - CONTRACTS_ADDRESS_MANAGER_ROLE: The role that can set the addresses of the contracts used by the VoterRewardsV3 contract.
 *
 * ------------------ Version 2 Changes ------------------
...
contract VoterRewardsV3 is AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
  ...
  /// @notice The role that can set the addresses of the contracts used by the VoterRewardsV3 contract.
  ...
  struct VoterRewardsStorage {
    IGalaxyMemberV3 galaxyMember;
    IB3TRV3 b3tr;
    IEmissionsV3 emissions;
  ...
  /// @notice Initialize the VoterRewardsV3 contract.
  ...
  function initialize(
    ...
    require(_galaxyMember != address(0), "VoterRewardsV3: _galaxyMember cannot be the zero address");
    require(_emissions != address(0), "VoterRewardsV3: emissions cannot be the zero address");
    require(_b3tr != address(0), "VoterRewardsV3: _b3tr cannot be the zero address");

    require(levels.length > 0, "VoterRewardsV3: levels must have at least one element");
    require(levels.length == multipliers.length, "VoterRewardsV3: levels and multipliers must have the same length");
    ...
    $.galaxyMember = IGalaxyMemberV3(_galaxyMember);
    $.b3tr = IB3TRV3(_b3tr);
    $.emissions = IEmissionsV3(_emissions);
    ...
    require(admin != address(0), "VoterRewardsV3: admin cannot be the zero address");
    _grantRole(DEFAULT_ADMIN_ROLE, admin);
    _grantRole(UPGRADER_ROLE, upgrader);
    _grantRole(CONTRACTS_ADDRESS_MANAGER_ROLE, contractsAddressManager);
  }

  /// @notice Upgrade the implementation of the VoterRewardsV3 contract.
  ...
  function registerVote(
    ...
    require(proposalStart > 0, "VoterRewardsV3: proposalStart must be greater than 0");
    require(voter != address(0), "VoterRewardsV3: voter cannot be the zero address");
  ...
  function claimReward(uint256 cycle, address voter) external nonReentrant {
    require(cycle > 0, "VoterRewardsV3: cycle must be greater than 0");
    require(voter != address(0), "VoterRewardsV3: voter cannot be the zero address");
    VoterRewardsStorage storage $ = _getVoterRewardsStorage();

    // Check if the cycle has ended before claiming rewards.
    require($.emissions.isCycleEnded(cycle), "VoterRewardsV3: cycle must be ended");

    // Get the reward for the voter in the cycle.
    uint256 reward = getReward(cycle, voter);

    require(reward > 0, "VoterRewardsV3: reward must be greater than 0");
    require($.b3tr.balanceOf(address(this)) >= reward, "VoterRewardsV3: not enough B3TR in the contract to pay reward");

    // Reset the reward-weighted votes for the voter in the cycle.
    $.cycleToVoterToTotal[cycle][voter] = 0;

    // transfer reward to voter
    require($.b3tr.transfer(voter, reward), "VoterRewardsV3: transfer failed");
  ...
  function getReward(uint256 cycle, address voter) public view returns (uint256) {
    ...
    require(emissionsAmount > 0, "VoterRewardsV3: emissionsAmount must be greater than 0");
  ...
  function galaxyMember() external view returns (IGalaxyMemberV3) {
    VoterRewardsStorage storage $ = _getVoterRewardsStorage();
    return $.galaxyMember;
  }

  /// @notice Get the Emissions contract.
  function emissions() external view returns (IEmissionsV3) {
    VoterRewardsStorage storage $ = _getVoterRewardsStorage();
    return $.emissions;
  }
  ...
  function setLevelToMultiplier(uint256 level, uint256 multiplier) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(level > 0, "VoterRewardsV3: level must be greater than 0");
    require(multiplier > 0, "VoterRewardsV3: multiplier must be greater than 0");
