  ...
  IXAllocationVotingGovernorV1
{
  ...
  function startNewRound() public virtual returns (uint256) {
    ...
    if (currentRound > 0) {
      require(!isActive(currentRound), "XAllocationVotingGovernorV1: there can be only one round per time");
    }
  ...
  function castVote(uint256 roundId, bytes32[] memory appIds, uint256[] memory voteWeights) public virtual {
    ...
    require(appIds.length == voteWeights.length, "XAllocationVotingGovernorV1: apps and weights length mismatch");
    require(appIds.length > 0, "XAllocationVotingGovernorV1: no apps to vote for");
