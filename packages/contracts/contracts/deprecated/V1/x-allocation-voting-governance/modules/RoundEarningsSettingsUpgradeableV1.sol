...
/**
 * @title RoundEarningsSettingsUpgradeableV1
 * @notice Extension of {XAllocationVotingGovernorV1} to handle the settings for the x-allocation earnings calculations:
 ...
 */
abstract contract RoundEarningsSettingsUpgradeableV1 is Initializable, XAllocationVotingGovernorV1 {
  ...
  function _setBaseAllocationPercentage(uint256 baseAllocationPercentage_) internal {
    require(
      ...
      "XAllocationVotingGovernorV1: Base allocation percentage must be less than or equal to 100"
    );
    EarningsSettingsStorage storage $ = _getEarningsSettingsStorage();
  ...
  function _setAppSharesCap(uint256 appSharesCap_) internal {
    require(appSharesCap_ <= 100, "XAllocationVotingGovernorV1: App shares cap must be less than or equal to 100");
    EarningsSettingsStorage storage $ = _getEarningsSettingsStorage();
