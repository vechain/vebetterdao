...
/**
 * @title ExternalContractsUpgradeableV1
 * @dev Extension of {XAllocationVotingGovernorV1} that handles the storage of external contracts for the XAllocationVotingGovernorV1.
 */
abstract contract ExternalContractsUpgradeableV1 is Initializable, XAllocationVotingGovernorV1 {
  ...
  function __ExternalContracts_init(
    ...
    IEmissionsV1 initialEmissions,
    IVoterRewardsV1 initialVoterRewards
   ...
   */
  function emissions() public view override returns (IEmissionsV1) {
    ExternalContractsStorage storage $ = _getExternalContractsStorage();
    return $._emissions;
  }
   ...
   */
  function voterRewards() public view override returns (IVoterRewardsV1) {
    ExternalContractsStorage storage $ = _getExternalContractsStorage();
   ...
   */
  function _setEmissions(IEmissionsV1 newEmisionsAddress) internal virtual {
    require(address(newEmisionsAddress) != address(0), "XAllocationVotingGovernorV1: emissions is the zero address");
    ExternalContractsStorage storage $ = _getExternalContractsStorage();

    emit EmissionsSet(address($._emissions), address(newEmisionsAddress));
    $._emissions = IEmissionsV1(newEmisionsAddress);
  }
  ...
  function _setX2EarnApps(IX2EarnAppsV1 newX2EarnApps) internal virtual {
    require(address(newX2EarnApps) != address(0), "XAllocationVotingGovernorV1: new X2EarnApps is the zero address");
   ...
   */
  function _setVoterRewards(IVoterRewardsV1 newVoterRewards) internal virtual {
    require(address(newVoterRewards) != address(0), "XAllocationVotingGovernorV1: new voter rewards is the zero address");
