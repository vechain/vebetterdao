...
/// @title GalaxyMemberV1 NFT Contract
  ...
  UUPSUpgradeable
{
  ...
  /// @notice Storage structure for GalaxyMemberV1
  ...
  struct GalaxyMemberStorage {
    ...
    IB3TRGovernorV1 b3trGovernor; // B3TRGovernor contract
  ...
  /// @notice Initializes a new GalaxyMemberV1 contract
  ...
  function upgrade(uint256 tokenId) external nonReentrant whenNotPaused {
    ...
    require($.b3tr.transferFrom(msg.sender, $.treasury, b3trRequired), "GalaxyMemberV1: Transfer failed");
  ...
  function setB3trGovernorAddress(address _b3trGovernor) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    ...
    $.b3trGovernor = IB3TRGovernorV1(payable(_b3trGovernor));
  }
  ...
  function participatedInGovernance(address user) public view returns (bool) {
    GalaxyMemberStorage storage $ = _getGalaxyMemberStorage();
    require(
      $.xAllocationsGovernor != IXAllocationVotingGovernorV1(address(0)),
      "Galaxy Member: XAllocationVotingGovernor not set"
    );
    require($.b3trGovernor != IB3TRGovernorV1(payable(address(0))), "Galaxy Member: B3TRGovernor not set");
  ...
  function b3trGovernor() external view returns (IB3TRGovernorV1) {
    GalaxyMemberStorage storage $ = _getGalaxyMemberStorage();
    return $.b3trGovernor;
  }
