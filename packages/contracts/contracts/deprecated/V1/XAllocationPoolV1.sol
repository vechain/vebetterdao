...
import { IXAllocationVotingGovernorV1 } from "./interfaces/IXAllocationVotingGovernorV1.sol";
...
contract XAllocationPoolV1 is IXAllocationPoolV1, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
  ...
  struct XAllocationPoolStorage {
    IXAllocationVotingGovernorV1 _xAllocationVoting;
    IEmissionsV1 _emissions;
    IB3TRV1 b3tr;
    ITreasuryV1 treasury;
    IX2EarnAppsV1 x2EarnApps;
    IX2EarnRewardsPoolV1 x2EarnRewardsPool;
  ...
  function initialize(
    ...
    require(_b3trAddress != address(0), "XAllocationPoolV1: new b3tr is the zero address");
    require(_treasury != address(0), "XAllocationPoolV1: new treasury is the zero address");
    require(_x2EarnApps != address(0), "XAllocationPoolV1: new x2EarnApps is the zero address");
    require(_x2EarnRewardsPool != address(0), "XAllocationPoolV1: new x2EarnRewardsPool is the zero address");
    ...
    $.b3tr = IB3TRV1(_b3trAddress);
    $.treasury = ITreasuryV1(_treasury);
    $.x2EarnApps = IX2EarnAppsV1(_x2EarnApps);
    $.x2EarnRewardsPool = IX2EarnRewardsPoolV1(_x2EarnRewardsPool);

    require(_admin != address(0), "XAllocationPoolV1: new admin is the zero address");
  ...
  function setXAllocationVotingAddress(address xAllocationVoting_) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(xAllocationVoting_ != address(0), "XAllocationPoolV1: new xAllocationVoting is the zero address");

    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    $._xAllocationVoting = IXAllocationVotingGovernorV1(xAllocationVoting_);
  ...
  function setEmissionsAddress(address emissions_) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(emissions_ != address(0), "XAllocationPoolV1: new emissions is the zero address");

    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    $._emissions = IEmissionsV1(emissions_);
  ...
  function setTreasuryAddress(address treasury_) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(treasury_ != address(0), "XAllocationPoolV1: new treasury is the zero address");

    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    $.treasury = ITreasuryV1(treasury_);
  ...
  function setX2EarnAppsAddress(address x2EarnApps_) external onlyRole(CONTRACTS_ADDRESS_MANAGER_ROLE) {
    require(x2EarnApps_ != address(0), "XAllocationPoolV1: new x2EarnApps is the zero address");
  ...
  function claim(uint256 roundId, bytes32 appId) external nonReentrant {
    ...
    require(!$.claimedRewards[appId][roundId], "XAllocationPoolV1: rewards already claimed for this app and round");
    require(!xAllocationVoting().isActive(roundId), "XAllocationPoolV1: round not ended yet");
    require($.x2EarnApps.appExists(appId), "XAllocationPoolV1: app does not exist");
    ...
    require(amountToClaim > 0, "XAllocationPoolV1: no rewards available for this app");
    ...
    require(
      ...
      "XAllocationPoolV1: Insufficient funds on contract"
    );
    ...
    require(
      ...
      "XAllocationPoolV1: transfer to team wallet failed"
    );
    ...
    require(
      ...
      "XAllocationPoolV1: Approval of B3TR token to x2EarnRewardsPool failed"
    );
    require(
      $.x2EarnRewardsPool.deposit(x2EarnRewardsPoolAmount, appId),
      "XAllocationPoolV1: Deposit of rewards allocation to x2EarnRewardsPool failed"
    );
    ...
    if (unallocatedAmount > 0) {
      require(
        ...
        "XAllocationPoolV1: Transfer of unallocated rewards to treasury failed"
      );
    }
  ...
  function _emissionAmount(uint256 roundId) internal view returns (uint256) {
    IEmissionsV1 _emissions = emissions();
    require(_emissions != IEmissionsV1(address(0)), "Emissions contract not set");
  ...
  {
    IXAllocationVotingGovernorV1 _xAllocationVoting = xAllocationVoting();

    require(_xAllocationVoting != IXAllocationVotingGovernorV1(address(0)), "XAllocationVotingGovernor contract not set");
    ...
    IXAllocationVotingGovernorV1.RoundState state = _xAllocationVoting.state(roundId);
    if (
      state == IXAllocationVotingGovernorV1.RoundState.Active || state == IXAllocationVotingGovernorV1.RoundState.Succeeded
    ) {
   ...
   */
  function xAllocationVoting() public view returns (IXAllocationVotingGovernorV1) {
    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
   ...
   */
  function emissions() public view returns (IEmissionsV1) {
    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    return $._emissions;
  }
   ...
   * @dev Returns the emissions contract.
   */
  function treasury() external view returns (ITreasuryV1) {
    XAllocationPoolStorage storage $ = _getXAllocationPoolStorage();
    return $.treasury;
  }
