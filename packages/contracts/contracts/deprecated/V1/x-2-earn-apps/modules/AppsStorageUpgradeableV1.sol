...
abstract contract AppsStorageUpgradeableV1 is Initializable, X2EarnAppsUpgradeableV1 {
  ...
  function _addApp(
    ...
    // Store the new app
    $._apps[id] = X2EarnAppsDataTypesV1.App(id, appName, block.timestamp);
    $._appIds.push(id);
   ...
   */
  function _getAppStorage(bytes32 appId) internal view returns (X2EarnAppsDataTypesV1.App memory) {
    if (!appExists(appId)) {
  ...
  // ---------- Getters ---------- //
  /**
   * @dev See {IX2EarnAppsV1-appExists}.
   */
  ...
  /**
   * @dev See {IX2EarnAppsV1-app}.
   *
   * @param appId the id of the app
   */
  function app(
    bytes32 appId
  ) public view virtual override returns (X2EarnAppsDataTypesV1.AppWithDetailsReturnType memory) {
    X2EarnAppsDataTypesV1.App memory _app = _getAppStorage(appId);

    return
      X2EarnAppsDataTypesV1.AppWithDetailsReturnType(
        _app.id,
  ...
  /**
   * @dev See {IX2EarnAppsV1-apps}.
   *
   * @notice This function could not be efficient with a large number of apps, in that case, use {IX2EarnAppsV1-getPaginatedApps}
   * and then call {IX2EarnAppsV1-app} for each app id
   */
  function apps() external view returns (X2EarnAppsDataTypesV1.AppWithDetailsReturnType[] memory) {
    AppsStorageStorage storage $ = _getAppsStorageStorage();

    uint256 length = $._appIds.length;
    X2EarnAppsDataTypesV1.AppWithDetailsReturnType[] memory allApps = new X2EarnAppsDataTypesV1.AppWithDetailsReturnType[](
      length
    );

    for (uint i = 0; i < length; i++) {
      X2EarnAppsDataTypesV1.App memory _app = $._apps[$._appIds[i]];
      allApps[i] = X2EarnAppsDataTypesV1.AppWithDetailsReturnType(
        _app.id,
  ...
  /**
   * @dev See {IX2EarnAppsV1-getPaginatedApps}.
   */
  function getPaginatedApps(uint startIndex, uint count) external view returns (X2EarnAppsDataTypesV1.App[] memory) {
    ...
    X2EarnAppsDataTypesV1.App[] memory paginatedApps = new X2EarnAppsDataTypesV1.App[](endIndex - startIndex);
  ...
  /**
   * @dev See {IX2EarnAppsV1-appsCount}.
   */
