  ...
  UUPSUpgradeable
{
  ...
  function initialize(InitializationData memory data) public initializer {
    require(address(data.vot3Token) != address(0), "XAllocationVotingV1: invalid VOT3 token address");
    require(address(data.voterRewards) != address(0), "XAllocationVotingV1: invalid VoterRewards address");
    require(address(data.emissions) != address(0), "XAllocationVotingV1: invalid Emissions address");
    
    __XAllocationVotingGovernor_init("XAllocationVotingV1");
    ...
    for (uint256 i; i < data.admins.length; i++) {
      require(data.admins[i] != address(0), "XAllocationVotingV1: invalid admin address");
  ...
  function votingPeriod() public view override(XAllocationVotingGovernorV1, VotingSettingsUpgradeableV1) returns (uint256) {
