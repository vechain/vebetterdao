...
contract VeBetterPassport is AccessControlUpgradeable, UUPSUpgradeable, IVeBetterPassport {
  ...
  function initialize(
    ...
    PassportConfigurator.initializePassportStorage(data);
  ...
  function userRoundActionCountApp(address user, uint256 round, bytes32 appId) external view returns (uint256) {
    return PassportPoPScoreLogic.userRoundActionCountApp(user, round, appId);
  }
  ...
  function minimumGalaxyMemberLevel() external view returns (uint256) {
    PassportStorageTypes.PassportStorage storage $ = PassportStorageTypes.getPassportStorage();
