...
abstract contract EndorsementUpgradeable is Initializable, X2EarnAppsUpgradeable {
  ...
  function allocateEndorsementPoints(bytes32 appId, uint256 nodeId, uint256 points) public virtual {
    EndorsementStorage storage $ = _getEndorsementStorage();
      //Check if points is greater than max, if so revert
      //Check if app exists, if not revert
      //Check if app is blacklisted, if so revert
      //Check if sender is node manager, if not revert
      //Check if node exists, if not revert
      //Check if node has available points, if not revert
      //Check if node is in cooldown (maybe per app? how will the cooldown play with the split endorsements?)
