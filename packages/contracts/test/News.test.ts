...
describe("News Contract", function () {
  ...
  beforeEach(async function () {
    ...
    // Submit the app
    await x2EarnApps.connect(owner).submitApp(appAdmin.address, appAdmin.address, appName, "ipfs://app-metadata")

    // Endorse the app
    await endorseApp(appId, appAdmin)

    // Set appModerator as a moderator for the app
    await x2EarnApps.connect(appAdmin).addAppModerator(appId, appModerator.address)
    ...
    await news.connect(owner).grantRole(await news.PUBLISHER_ROLE(), publisher.address)

    // Grant moderator role
    await news.connect(owner).grantRole(await news.MODERATOR_ROLE(), moderator.address)
  })

  describe("Initialization", function () {
    it("Should initialize correctly", async function () {
      expect(await news.cooldownPeriod()).to.equal(cooldownPeriod)
      expect(await news.x2EarnApps()).to.equal(await x2EarnApps.getAddress())

      // Check roles
      expect(await news.hasRole(await news.DEFAULT_ADMIN_ROLE(), owner.address)).to.be.true
      expect(await news.hasRole(await news.PAUSER_ROLE(), pauser.address)).to.be.true
      expect(await news.hasRole(await news.UPGRADER_ROLE(), upgrader.address)).to.be.true
      expect(await news.hasRole(await news.PUBLISHER_ROLE(), publisher.address)).to.be.true
      expect(await news.hasRole(await news.MODERATOR_ROLE(), moderator.address)).to.be.true
    })
  ...
  describe.only("Publishing News", function () {
    it("Publisher can publish news", async function () {
      const { title, description, image, callToActionUrl } = TEST_NEWS.publisher

      const transaction = await news.connect(publisher).publish(appId, title, description, image, callToActionUrl)
    ...
    it("App admin can publish news", async function () {
      const { title, description, image, callToActionUrl } = TEST_NEWS.admin
    ...
    it("App creator can publish news", async function () {
      ...
      const { title, description, image, callToActionUrl } = TEST_NEWS.creator
    ...
    it("App moderator can publish news", async function () {
      const { title, description, image, callToActionUrl } = TEST_NEWS.moderator
    ...
    it("Regular user cannot publish news", async function () {
      ...
      await expect(news.connect(user).publish(appId, title, description, image, callToActionUrl)).to.be.revertedWith(
    ...
    it("Should respect cooldown period for app publishers", async function () {
      ...
      await expect(
        news
          .connect(appAdmin)
          .publish(appId, secondNews.title, secondNews.description, secondNews.image, secondNews.callToActionUrl),
