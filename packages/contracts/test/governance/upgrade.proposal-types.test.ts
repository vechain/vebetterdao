...
describe.only("Proposal Type Storage", function () {
  it("Should preserve proposal data through version upgrades and add proposal type support", async () => {
    ...
    // Upgrade V5 -> V6 (current version with proposal types)
    const governorV6 = (await upgradeProxy(
      "B3TRGovernorV5",
      "B3TRGovernor",
      governorContractAddress,
      [
    ...
    // Verify all proposals persisted through V5 -> V6 upgrade
    expect(await governorV6.proposalProposer(proposalIdV1)).to.equal(proposerV1)
    expect(await governorV6.state(proposalIdV1)).to.equal(ethers.toBigInt(7)) //At this stage should be a failed proposal
    expect(await governorV6.proposalProposer(proposalIdV2)).to.equal(proposer.address)
    expect(await governorV6.state(proposalIdV2)).to.equal(ethers.toBigInt(7)) //At this stage should be a failed proposal
    expect(await governorV6.proposalProposer(proposalIdV5)).to.equal(proposer.address)
    expect(await governorV6.state(proposalIdV5)).to.equal(ethers.toBigInt(0))

    // Test proposal type functionality - old proposals should default to Standard (0)
    expect(await governorV6.proposalType(proposalIdV1)).to.equal(ethers.toBigInt(0)) // Standard type
    expect(await governorV6.proposalType(proposalIdV2)).to.equal(ethers.toBigInt(0)) // Standard type
    expect(await governorV6.proposalType(proposalIdV5)).to.equal(ethers.toBigInt(0)) // Standard type

    // Create new proposals in V6 with explicit proposal types
    await waitForBlock(1)
    const roundIdforV6 = await startNewRoundAndGetRoundId(emissions, xAllocationVoting)

    const txV6 = await governorV6.connect(proposer).propose([], [], [], "descriptionV6", roundIdforV6, 0, {
      gasLimit: 10_000_000,
    })
    const proposeReceiptV6 = await txV6.wait()
    const eventV6 = proposeReceiptV6?.logs[0]

    const decodedV6Logs = governorV6.interface.parseLog({
      topics: [...(eventV6?.topics as string[])],
      data: eventV6 ? eventV6.data : "",
    })

    const proposalIdV6 = ethers.toBigInt(decodedV6Logs?.args[0])

    // Verify proposal exists and get initial data
    const proposerV6 = await governorV6.proposalProposer(proposalIdV6)
    const stateV6 = await governorV6.state(proposalIdV6)

    expect(proposerV6).to.equal(proposer.address)
    expect(stateV6).to.equal(ethers.toBigInt(0)) // Pending state

    // Verify new proposals have correct types
    expect(await governorV6.proposalType(proposalIdV6)).to.equal(ethers.toBigInt(0))

    // Verify all proposal data is still accessible
    expect(await governorV6.proposalProposer(proposalIdV6)).to.equal(proposer.address)
    expect(await governorV6.state(proposalIdV6)).to.equal(ethers.toBigInt(0))
  })
  ...
  it("Should fallback to old quorum numerator if timepoint is before the upgrade", async () => {
    ...
    const v5QuorumNumerator = await governorV5["quorumNumerator()"]()
    const v5QuorumNumeratorBlockNumber = await ethers.provider.getBlockNumber()
    ...
    const newQuorumNumerator = v5QuorumNumerator + 2n
    await governorV6.connect(owner).updateQuorumNumeratorByType(newQuorumNumerator, proposalStandardType)

    const v6QuorumNumeratorLatest = await governorV6["quorumNumerator()"]()
    const v6QuorumNumeratorByTypeAtBlock = await governorV6["quorumNumeratorByType(uint256,uint8)"](
      v5QuorumNumeratorBlockNumber,
      proposalStandardType,
    )
    const v6QuorumNumeratorByTypeLatest = await governorV6["quorumNumeratorByType(uint8)"](proposalStandardType)

    //Querying without a timepoint should return the latest quorum numerator
    expect(v6QuorumNumeratorLatest).to.be.equal(newQuorumNumerator)

    //Querying with a timepoint should return the quorum numerator at that timepoint
    //Which in this case is before the upgrade, so should be the same as the v5 quorum numerator
    expect(v6QuorumNumeratorByTypeAtBlock).to.be.equal(v5QuorumNumerator)
