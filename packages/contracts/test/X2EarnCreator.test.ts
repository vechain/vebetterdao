...
describe("X2EarnCreator - @shard11", () => {
  ...
  describe("Upgrading", () => {
    ...
    it("V1 to V2 upgrade should preserve storage and enable selfMint", async function () {
      ...
      const v2 = (await upgradeProxy("X2EarnCreatorV1", "X2EarnCreator", await v1.getAddress(), [], {
        version: 2,
      })) as X2EarnCreator
      ...
      expect(await v2.selfMintEnabled()).to.equal(false)

      await v2.connect(owner).setSelfMintEnabled(true)
  ...
  describe("Self Minting", () => {
    it("Should allow self-minting when enabled", async () => {
      const { x2EarnCreator, owner, otherAccounts } = await getOrDeployContractInstances({ forceDeploy: true })

      const user = otherAccounts[15]

      await x2EarnCreator.connect(owner).setSelfMintEnabled(true)
    ...
    it("Should revert selfMint when disabled", async () => {
      const { x2EarnCreator, otherAccounts } = await getOrDeployContractInstances({ forceDeploy: true })

      const user = otherAccounts[15]

      expect(await x2EarnCreator.selfMintEnabled()).to.equal(false)
    ...
    it("Only admin should be able to set selfMintEnabled", async () => {
      const { x2EarnCreator, otherAccount } = await getOrDeployContractInstances({ forceDeploy: true })

      await expect(x2EarnCreator.connect(otherAccount).setSelfMintEnabled(true)).to.be.reverted
    })
  })
})
