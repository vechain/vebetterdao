...
describe("VOT3", function () {
  ...
  describe("Voting power", function () {
    ...
    it("Voting power should be the square root of the amount of vote", async function () {
      ...
      expect(await vot3Contract.getVotingPower(other)).to.eql(ethers.parseEther("31.622776601") / 1000000000n)
    })
    it("Voting power should be the square root of the amount of delegated votes", async function () {
      ...
      expect(await vot3Contract.getVotingPower(accounts[2])).to.eql(ethers.parseEther("4") / 1000000000n)
    })
    it("Should be able to get past voting power", async function () {
      ...
      expect(await vot3Contract.getVotingPower(accounts[3])).to.eql(ethers.parseEther("223.606797749") / 1000000000n)
      ...
      expect(await vot3Contract.getVotingPower(accounts[3])).to.eql(ethers.parseEther("200") / 1000000000n)

      // Voting power should be the same as before the transfer
      expect(await vot3Contract.getPastVotingPower(accounts[3], receipt.blockNumber - 1)).to.eql(
