...
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| 29 June 2026        | `VoterRewards` version `8`                                                                                                    | Fix navigator/relayer fee gates in `claimReward` / `_getRewardsAndFees` to use snapshot delegation state (`isDelegatedAtTimepoint`) instead of current state; decouple `registerRelayerAction(CLAIM)` from `relayerFee > 0`. Unblocks RelayerRewardsPool rounds when a citizen un-delegates (or their navigator exits) between snapshot and claim. Affected mainnet rounds: 101, 102. |
